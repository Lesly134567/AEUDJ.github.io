...
if ($check->fetch()) {
    header("Location: lista.php?visto=1");
    exit;
}
...
if ($_POST) {
    ...
    // Validar máximo 1 por dirección
    $idas = 0; $vueltas = 0;
    ...
    // Verificar duplicados por horario exacto
    foreach ($horarios as $horario) {
        $check = $pdo->prepare("SELECT id FROM votos WHERE usuario_id = ? AND horario = ? AND fecha = CURDATE()");
        $check->execute([$_SESSION['user_id'], $horario]);
        if ($check->fetch()) {
    ...
    // Insertar votos 
    foreach ($horarios as $horario) {
        $stmt = $pdo->prepare("INSERT INTO votos (usuario_id, horario, fecha) VALUES (?,?,CURDATE())");
        $stmt->execute([$_SESSION['user_id'], $horario]);
    }
...
<script>
  ...
  function toggleSlot(el, fullText, direction) {
    ...
    // Deshabilitar los demás de la misma dirección
    ...
    showStatus(`✅ Viaje de ${direction === 'ida' ? 'ida' : 'vuelta'} seleccionado`, 'text-green-600');
  ...
  function showStatus(message, className) {
    ...
    statusEl.className = `mt-4 text-sm font-medium ${className}`;
  }
</script>
</body>
</html>